...
def ice_masking():
    ...
    #CREATE the ICE MASK 
    MSK = xr.open_dataset('/Users/dashakan/Desktop/scripts_AIS/MARcst-AN35km-176x148.cdf2'
    ...
    test=MSK
    shf=(test['ICE']-test['GROUND']).where( (test['ICE']>30) & (test['ICE']-test['GROUND']>0) )
    shf_msk=(shf*test['AREA'])/100

    lsm = (MSK['AIS']<1)
    ground=(MSK['GROUND']*MSK['AIS']>30)
    shelf=shf*MSK['AIS']>30
    ...
    return MSK,ice,lsm,ground,shelf,x2D,y2D,sh,dh,rock,ice_msk,grd_msk


def grat_extend():
    MARbig = xr.open_dataset('/Users/charlesamory/Desktop/Work/python/map_sumup/data/MARcst-ANT35km-201x171.cdf'
                       ,decode_times=False)
...
def def_cmaps():
    ...
    cmap_div = plt.get_cmap('RdBu') # RdBu_r
...
def plot_cbar2(gs,cmap,bounds,extend='both'):
    axb1 = plt.subplot(gs[1])
    if extend=='both':
        ticks = bounds[1:-1]
    elif extend=='max':
        ...
        print ('Error in plot_cbar, extend keyword')
        return
    mpl.colorbar.ColorbarBase(axb1,cmap=cmap,extend=extend,ticks=ticks)

def top_label(label):
    plt.text(0,2900,label,ha='center',va='top',fontsize='large')
...
def spcolor(x2D,y2D,var2D,cmap,bounds,dh,sig):
    ...
    if sig=='sig':
        ...
        grey1='#BEBEBE'
        plt.pcolormesh(x2D+dh, y2D+dh,var2D,cmap=cmap,hatch='////',edgecolor=grey1,linewidth=0.0,norm=norm_cmap(cmap,bounds))
#    axisminmax(axis_minmax)
        
def spcolor2(x2D,y2D,var2D,cmap,dh,sig,vmin,vmax):

    if sig=='sig':
        plt.pcolormesh(x2D+dh, y2D+dh,var2D,cmap=cmap,vmin=vmin,vmax=vmax)
    else:
        grey1='#BEBEBE'
        plt.pcolormesh(x2D+dh, y2D+dh,var2D,hatch='////',edgecolor=grey1,linewidth=0.0,cmap=cmap,vmin=vmin,vmax=vmax)
